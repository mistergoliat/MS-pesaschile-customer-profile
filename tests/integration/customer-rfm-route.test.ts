...
describe('GET /v1/customers/:masterCustomerId/rfm', () => {
  it('returns 200 with metrics, scores, segment and snapshot metadata for a found customer', async () => {
    ...
    const response = await fetch(`${baseUrl}/v1/customers/9001/rfm`);
  ...
  it('returns 404 when the master customer does not exist', async () => {
    ...
    expect((await fetch(`${baseUrl}/v1/customers/9999/rfm`)).status).toBe(404);
  });
  ...
  it('returns 503 when no published RFM snapshot exists', async () => {
    ...
    expect((await fetch(`${baseUrl}/v1/customers/9001/rfm`)).status).toBe(503);
  });
  ...
  it('returns 400 for invalid masterCustomerId, query params or JSON body without calling the use case', async () => {
    ...
    expect((await fetch(`${baseUrl}/v1/customers/9001/rfm?foo=bar`)).status).toBe(400);
    expect(await rawGetWithJsonBody(`${baseUrl}/v1/customers/9001/rfm`)).toBe(400);
  ...
  it('does not implement in-repo auth and keeps the same behavior with or without Authorization headers', async () => {
    ...
    expect((await fetch(`${baseUrl}/v1/customers/9001/rfm`)).status).toBe(200);
    expect(
      (
        await fetch(`${baseUrl}/v1/customers/9001/rfm`, {
