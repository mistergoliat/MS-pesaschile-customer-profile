...
describe('Audience context, SQL, and evaluation result', () => {
  ...
  it('compiles fixed SELECT-only SQL with expression parameters before snapshot parameters', () => { const compiled = compileAudienceSql({ ...context, lineage: { ...context.lineage, feature: { ...context.lineage.feature, snapshotId: '2' }, rfm: { ...context.lineage.rfm!, snapshotId: '1' }, cluster: { ...context.lineage.cluster!, snapshotId: '1' }, clv: { ...context.lineage.clv!, snapshotId: '1' }, commercialAffinity: { ...context.lineage.commercialAffinity!, snapshotId: '4' } } }, scalar('rfm.recencyDays', 'GTE', 180) as any); expect(compiled.sql).toMatch(/^SELECT/); expect(compiled.sql).toContain('customer_feature_snapshot_row'); expect(compiled.sql).toContain('rr.recency_days >= ?'); expect(compiled.sql).not.toContain('customer.email'); expect(compiled.params).toEqual([180, '1', '1', '1', '2']); });
  ...
  it('accepts an RFM segment code paired with the resolved segment version', () => { const result = evaluateAudienceRows(definition({ kind: 'AND', children: [scalar('rfm.segmentCode', 'EQ', 'AT_RISK_HIGH_VALUE'), scalar('rfm.segmentVersion', 'EQ', 'segment-v1')] }), [row(1)], { context }); expect(result.status).toBe('completed'); });
});
