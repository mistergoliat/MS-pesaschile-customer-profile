...
function toReadRow(row: RowDataPacket): AudiencePreviewReadRow {
  ...
  return {
    ...
    ...(hasAffinity ? { affinity: { axis: row.affinityAxis as 'PRODUCT_FAMILY' | 'DISCIPLINE' | 'USE_CONTEXT', code: String(row.affinityCode), score: String(row.affinityScore), supportingOrderCount: Number(row.supportingOrderCount), supportingProductCount: Number(row.supportingProductCount), supportingUnits: row.supportingUnits === null || row.supportingUnits === undefined ? null : Number(row.supportingUnits), supportingSpend: String(row.supportingSpend), lastEvidenceAt: toIso(row.lastEvidenceAt), explicitEvidenceCoverage: row.explicitEvidenceCoverage === null ? null : String(row.explicitEvidenceCoverage) } } : {}),
  };
}
